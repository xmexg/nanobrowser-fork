      ...
      <div
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className={`text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                {t('options_general_viewportMode')}
              </h3>
              <p className={`text-sm font-normal ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                {t('options_general_viewportMode_desc')}
              </p>
            </div>
            <select
              id="viewportMode"
              value={settings.viewportMode}
              onChange={e => updateSetting('viewportMode', e.target.value as 'auto' | 'visible' | 'all')}
              className={`w-44 rounded-md border ${isDarkMode ? 'border-slate-600 bg-slate-700 text-gray-200' : 'border-gray-300 bg-white text-gray-700'} px-3 py-2 text-sm`}>
              <option value="auto">{t('options_general_viewportMode_auto')}</option>
              <option value="visible">{t('options_general_viewportMode_visible')}</option>
              <option value="all">{t('options_general_viewportMode_all')}</option>
            </select>
          </div>
